Ext.define('Twotest.twotest.web.com.controller.defaultcontext.defaultdomain.AAMainController', {
     extend: 'Twotest.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AAMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AATree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#AAGrid');
               grid.store.add(data);
               var tree = this.view.down('#AATree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#AA');
          var grid = this.view.down('#AAGrid');
          var tree = this.view.down('#AATreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#AAGrid');
               ...
               var formPanel = this.getView().up().down("#AA");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#AATree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AA').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AAGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.aId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#AA').getViewModel().setData(responseData.response.data);
     },
