Ext.define('Twotest.twotest.web.com.controller.defaultcontext.defaultdomain.FdfdMainController', {
     extend: 'Twotest.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.FdfdMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#FdfdTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#FdfdGrid');
               grid.store.add(data);
               var tree = this.view.down('#FdfdTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Fdfd');
          var grid = this.view.down('#FdfdGrid');
          var tree = this.view.down('#FdfdTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#FdfdGrid');
               ...
               var formPanel = this.getView().up().down("#Fdfd");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#FdfdTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Fdfd').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#FdfdGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.sd;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#Fdfd').getViewModel().setData(responseData.response.data);
     },
