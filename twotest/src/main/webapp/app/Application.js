 ...
 */
Ext.define('Twotest.Application', {
	...
	requires : [ "Ext.ux.BoxReorderer", "Ext.ux.CellDragDrop",
			...
			"Ext.ProgressBarWidget", "Ext.ZIndexManager","Twotest.view.databrowsercalendar.DBCalendar" ],
	...
	stores : [ 'Twotest.store.QueryCriteriaWidgetsStore' ],
...
Ext.Ajax.on('requestcomplete', function(conn, response, options, eOpts) {
	...
	if (responseData.response) {
		if (responseData.response.exceptionId == 4005 && !sessionTimeOutFlag) {
			...
			var loginWindow = Ext.create('Twotest.view.login.SessionLogin')
					.center();
